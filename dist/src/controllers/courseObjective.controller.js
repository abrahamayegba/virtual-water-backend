...
exports.courseObjectiveController = void 0;
const prisma_1 = require("../lib/prisma");
exports.courseObjectiveController = {
    ...
    createObjective: async (req, res) => {
        try {
            ...
            const newObjective = await prisma_1.prisma.courseObjective.create({
    ...
    getObjectiveById: async (req, res) => {
        try {
            const { id } = req.params;
            const objective = await prisma_1.prisma.courseObjective.findUnique({
                where: { id },
                include: { course: true },
            });
            if (!objective) {
    ...
    updateObjective: async (req, res) => {
        try {
            const { id } = req.params;
            const { objective, courseId } = req.body;
            const updated = await prisma_1.prisma.courseObjective.update({
                where: { id },
                data: { objective, courseId },
            });
    ...
    deleteObjective: async (req, res) => {
        try {
            const { id } = req.params;
            await prisma_1.prisma.courseObjective.delete({ where: { id } });
