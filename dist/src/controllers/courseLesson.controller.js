...
exports.courseLessonController = {
    // create lesson
    createLesson: async (req, res) => {
        try {
            ...
            const lessonType = await prisma_1.prisma.lessonType.findUnique({
                where: { id: Number(typeId) },
            });
            if (!lessonType)
                return res.status(404).json({ message: "Lesson type not found" });
            const lesson = await prisma_1.prisma.courseLesson.create({
                data: {
                    title,
                    content,
    ...
    getLessonById: async (req, res) => {
        try {
            const { id } = req.params;
            const lesson = await prisma_1.prisma.courseLesson.findUnique({
                where: { id },
                include: { type: true, course: true },
            });
            if (!lesson)
    ...
    getLessonsByUserCourseId: async (req, res) => {
    ...
    updateLesson: async (req, res) => {
        try {
            const { id } = req.params;
            const { title, content, typeId, duration, file, courseId } = req.body;
            const lesson = await prisma_1.prisma.courseLesson.update({
                where: { id },
                data: {
                    ...(title && { title }),
                    ...(content && { content }),
    ...
    deleteLesson: async (req, res) => {
        try {
            const { id } = req.params;
            await prisma_1.prisma.courseLesson.delete({ where: { id } });
